...
@injectable
class SubmitGameAnswerCubit extends Cubit<BaseApiState<ThinkAlikeSession>> {
  SubmitGameAnswerCubit(this.repository) : super(const BaseApiState.initial());
  final ThinkAlikeRepository repository;
  Future<void> submit(int id, String answer) async {
    emit(const BaseApiState.loading());
    final r = await repository.answer(id, answer);
