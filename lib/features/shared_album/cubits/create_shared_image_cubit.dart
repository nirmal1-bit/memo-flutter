...
class CreateSharedImageCubit extends Cubit<BaseApiState<SharedImageResponse>> {
  CreateSharedImageCubit(this.sharedAlbumRepository) : super(const BaseApiState.initial());
  final SharedAlbumRepository sharedAlbumRepository;
  Future<void> createSharedImage(int connectionId, SharedImageRequest request) async {
    emit(const BaseApiState.loading());
    final response = await sharedAlbumRepository.createSharedImage(connectionId, request);
